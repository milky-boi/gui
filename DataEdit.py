...
class DataEdit_:
    ...
    def get_stats(self):
        ...
        df['mean'] = df.iloc[:,11:].mean(axis=1)
        df['se'] = df.iloc[:,11:].sem(axis=1)
    ...
    def split_for_graph(self, start_bsl_morning, start_1st_expo, start_bsl_noon, start_2nd_expo):
        ...
        """
        messagebox.showinfo("The data has been processed",
                            "You can see the results of the data processing by clicking the buttons Individual, "
                            "Population and Hour average, located at the top of the screen")
        ...
        df_1_edited = df_1.drop([1, 2, 3, 4, 5, 6, 7], axis=1)

        df_info = df_1_edited.iloc[:, :4]
        df_data = df_1_edited.iloc[:, 4:]
        ...
        df_1_edited = pd.concat([df_info, df_data], axis=1, sort=False)


        df_2_edited = df_2.drop([1, 2, 3, 4, 5, 6, 7], axis=1)
        df_info = df_2_edited.iloc[:, :4]
        df_data = df_2_edited.iloc[:, 4:]
        ...
        df_2_edited = pd.concat([df_info, df_data], axis=1, sort=False)


        df_3_edited = df_3.drop([1, 2, 3, 4, 5, 6, 7], axis=1)
        df_info = df_3_edited.iloc[:, :4]
        df_data = df_3_edited.iloc[:, 4:]
        ...
        df_3_edited = pd.concat([df_info, df_data], axis=1, sort=False)



        df_4_edited = df_4.drop([1, 2, 3, 4, 5, 6, 7], axis=1)
        df_info = df_4_edited.iloc[:, :4]
        df_data = df_4_edited.iloc[:, 4:]
        ...
        df_4_edited = pd.concat([df_info, df_data], axis=1, sort=False)



        with pd.ExcelWriter('results/exp_split.xlsx') as writer:
            df_1_edited.to_excel(writer, sheet_name='bsl_morning')
            df_2_edited.to_excel(writer, sheet_name='1st_adm')
            df_3_edited.to_excel(writer, sheet_name='bsl_noon')
            df_4_edited.to_excel(writer, sheet_name='2nd_amd')
